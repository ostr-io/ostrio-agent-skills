...
//
// Self-managed Next.js middleware for ostr.io pre-rendering.
// Zero extra dependencies. Copy as-is and set PRERENDER_SERVICE_AUTH / OSTR_AUTH.
//
// The BOT_AGENTS and IGNORE_EXTENSIONS arrays below are copied byte-for-byte
// from the canonical sources. Sync from upstream whenever new AI agents emerge:
...
//
// Canonical upstream example:
//   https://github.com/ostr-io/ostrio-docs/blob/master/docs/prerendering/examples/next.js/middleware.ts

import type { NextRequest } from 'next/server';
import { NextResponse } from 'next/server';

// [IMPORTANT!]
// Set PRERENDER_SERVICE_AUTH, SPIDERABLE_SERVICE_AUTH or OSTR_AUTH env var to token
// that starts with `Basic ...base64 encoded string...` from the pre-rendering panel.
// DO NOT hardcode it — set it as a Vercel / deployment secret.
...
// Deprecated by Google in 2015 but still used by non-Google crawlers.
const SUPPORT_ESCAPED_FRAGMENT = true;

// Logger facility (console by default).
const LOGGER = console;

// Set to false to strip query strings before sending to the renderer.
// Cleaner cache keys — set to false only if the app doesn't route on query strings.
const PRERENDER_WITH_QUERY = true;

// Project-specific ignore regex. Example: /^\/(admin\/|assets\/|preview\/)/i
const IGNORED_PATHS_RE: RegExp | false = false;

// Canonical static-extensions set — keep in sync with:
// https://github.com/ostr-io/ostrio-docs/blob/master/docs/prerendering/shared/static-extensions-regex.md
...
// Canonical crawler UA list — keep in sync with:
// https://github.com/ostr-io/ostrio-docs/blob/master/docs/prerendering/shared/crawler-ua-regex.md
...
if (!AUTH_HEADER) {
  throw new Error('ostr.io pre-rendering middleware requires AUTH TOKEN — set PRERENDER_SERVICE_AUTH, SPIDERABLE_SERVICE_AUTH, or OSTR_AUTH env var.');
}

// @see https://nextjs.org/docs/app/api-reference/file-conventions/middleware#matcher
// The matcher must be static — Next.js compiles it at build time.
// Default excludes: API routes, Next.js static assets, HMR websocket, .well-known, favicon.
export const config = {
...
function shouldPrerender(req: NextRequest, url: URL, ua: string): boolean {
  if (!DICT.allowedMethods.has(req.method)) return false;
...
async function sendRequest(url: URL, headers: Headers, attempt = 1): Promise<Response | null> {
  ...
  try {
    ...
    if (nextAttempt <= 2) {
      LOGGER.info('[ostr.io] [middleware] [sendRequest] RETRY');
      return await sendRequest(url, headers, nextAttempt);
    }
    LOGGER.warn('[ostr.io] [middleware] [sendRequest] ERROR;', (requestError as any)?.cause ?? requestError);
  }
  return null;
}
...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    LOGGER.warn('[ostr.io] [middleware] CAUGHT ERROR; FALLBACK TO NextResponse.next()', (fetchError as any)?.cause ?? fetchError);
    return NextResponse.next();
  }
}
